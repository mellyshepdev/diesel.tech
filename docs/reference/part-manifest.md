...
| Mechanic career ladder: `coins`/`mechanicLevel` state (EngineViewer.tsx), 6-tier `LEVELS`/`REPAIRS` gating, Tool Shop (`TOOL_PRICES`/`ownedTools` in ToolPanel.tsx — specialty tools bought in the drawer they already live in) | n/a (game-progression state, not scene geometry) | n/a | coins + owned tools persisted to `localStorage` (`diesel-tech-coins`, `diesel-tech-owned-tools`); only `filterWrench`/`lineWrench` are priced since they're the only specialty tools an existing procStep actually checks for | 2026-07-20 |
